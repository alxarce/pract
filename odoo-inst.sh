...
sudo mkdir /etc/opt/odoo && sudo mkdir /var/log/odoo/ && sudo mkdir /var/opt/odoo/

# Instalamos Postgres y librerías base del sistema:
sudo apt-get update && sudo apt-get install postgresql-10 postgresql-server-dev-10 build-essential python3-pil python3-lxml python-ldap3 python3-dev python3-pip python3-setuptools npm nodejs git gdebi libldap2-dev libsasl2-dev  libxml2-dev libxslt1-dev libjpeg-dev -y
...
sudo chown odoo:odoo /opt/odoo/ -R && sudo chown odoo:odoo /var/log/odoo/ -R && sudo chown odoo:odoo /var/opt/odoo/ -R && cd /opt/odoo/odoo && sudo pip3 install -r requirements.txt
...
sudo mv /opt/odoo/.odoorc /etc/opt/odoo/odoo.conf

#Agregamos los siguientes parámetros al archivo de configuración de odoo:
sudo sed -i "s,^\(logfile = \).*,\1"/var/log/odoo/odoo-server.log"," /etc/opt/odoo/odoo.conf
#sudo sed -i "s,^\(logrotate = \).*,\1"True"," /etc/odoo/odoo.conf
#sudo sed -i "s,^\(proxy_mode = \).*,\1"True"," /etc/odoo/odoo.conf

#Agregamos los siguientes parámetros al archivo de configuración de postgres:
sudo sed -i "663ilisten_addresses = '*'" /etc/postgresql/10/main/postgresql.conf
sudo sed -i "101ihost    all             all             0.0.0.0/0               md5" /etc/postgresql/10/main/pg_hba.conf
